...
const MyProducts = () => {
    ...
    useEffect(() => {
        async function getProducts() {
            const url = `https://blooming-earth-59051.herokuapp.com/productlist?email=${user.email}`;
            try {
                ...
                if (err.response.status === 401 || err.response.status === 403) {
                    ...
                    console.log("Error :(");
                    toast.warn(
                        "Please Login again with Email and Password to show your products..."
                    );
                    signOut(auth);
                    navigate("/login");
                }
    ...
    const handleDelete = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                const url = `https://blooming-earth-59051.herokuapp.com/product/${id}`;
                fetch(url, {
                    method: "DELETE",
                })
                    .then((res) => res.json())
